...
export type ParamType =
  ...
  | 'record' // pick a single record of a collection (collectionFrom) → its id
  | 'association' // pick a to-many relation defined on a collection → {name, source, target, titleField}
  | 'popupView' // pick an existing View action (popup) on the surrounding table → its uid
  | 'filterOptions' // build named filter options (label + native-filter condition group, with test-run)
  | 'enumOptions' // pick values from a chosen field's native enum options → [{value,label,color}]
  | 'fieldValue' // a value for a chosen field: enum → select, boolean → switch, else input
  | 'text'
  | 'code' // multi-line monospace editor (SQL / JS)
  | 'color' // color picker (hex)
  | 'theme' // visual theme swatch wall → theme key string
  | 'styleSelect' // visual variant picker — option cards with thumbnails (see core/styleThumbs)
  | 'number'
  | 'select'
  | 'boolean';
